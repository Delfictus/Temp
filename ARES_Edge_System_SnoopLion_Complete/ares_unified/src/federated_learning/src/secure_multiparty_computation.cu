...
private:
    ...
    bool verifyMACs(const std::vector<SecretShare>& shares) {
        ...
        // Check all verifications passed - using simple reduction instead of thrust::all_of
        bool all_valid = true;
        bool* h_results = new bool[num_parties_];
        cudaMemcpy(h_results, thrust::raw_pointer_cast(d_results.data()), 
                   num_parties_ * sizeof(bool), cudaMemcpyDeviceToHost);
    ...
    void sendMessage(uint32_t party_id, const std::vector<uint8_t>& message) {
        ...
        total_communication_bytes_ = total_communication_bytes_.load() + msg_size;
    }
