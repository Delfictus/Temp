...
struct DisruptionPlan {
    ...
    std::vector<uint32_t> primary_targets;
    std::vector<uint32_t> secondary_targets;
    float execution_confidence;
    uint64_t start_time_ns;
    uint64_t duration_ns;
    std::unordered_map<uint32_t, uint32_t> signature_swaps;  // member_id -> impersonate_id
    std::vector<float3> false_target_positions;
};
